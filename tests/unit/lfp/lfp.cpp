...
BOOST_AUTO_TEST_CASE(LFP_PointSource_LineSource) {
  ...
  for (auto k = 0; k < 10; k++) {
    ...
    double analytic_approaching_lfp =
        line_source_lfp_factor(approaching_elec, segment_start, segment_end,
                               floor, medium_resistivity_fac);
    double analytic_circling_lfp =
        line_source_lfp_factor(circling_elec, segment_start, segment_end, floor,
                               medium_resistivity_fac);
    double numeric_circling_lfp =
        integral([&](double x) {
                   return 1.0 /
                          std::max(floor,
                                   norm(
    ...
    BOOST_REQUIRE((approaching_elec[1] < 0.866e-6)
                      ? analytic_approaching_lfp == 1.0e6
                      : true);
  ...
  std::vector<std::array<double, 3> > electrodes = { { 0.0, 0.3, 0.0 },
                                                     { 0.0, 0.7, 0.8 } };
  std::vector<int> indices = { 0, 1, 2, 3 };
  LFPCalculator<LineSource> lfp(segments_starts, segments_ends, radii, indices,
                                electrodes, 1.0);
  lfp.template lfp<std::vector<double> >({ 0.0, 1.0, 2.0, 3.0 });
  std::vector<double> res_line_source = lfp.lfp_values;
  LFPCalculator<PointSource> lfpp(segments_starts, segments_ends, radii,
                                  indices, electrodes, 1.0);
